...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   nm_get_sect_type_32.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cledant <cledant@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/06 13:07:18 by cledant           #+#    #+#             */
/*   Updated: 2017/02/06 13:10:16 by cledant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "nm.h"

static void		compare_text(t_sect_data *data, const struct section *sec)
{
					...
					const struct segment_command *seg)
{
	...
	while (i <= seg->nsects)
	{
		if (nm_is_interval_valid((size_t)sec, sizeof(struct section), info)
				== NM_FAIL)
			return ;
...
static int		seek_type(t_sect_data *data, const t_info *info,
					const struct segment_command *seg)
{
	if (nm_is_interval_valid((size_t)seg, sizeof(struct segment_command),
			info) == NM_FAIL)
		return (NM_FAIL);
					...
					const struct mach_header *m_header, const t_info *info)
{
	...
	lc = (void *)m_header + sizeof(struct mach_header);
