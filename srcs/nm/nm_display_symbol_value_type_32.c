			...
			const t_info *info)
{
	...
	if (sy_info.stab != 0)
		...
		nm_print_absolute_32(MAJ, data[item.cur].n_value);
	else if (sy_info.type == N_ABS && sy_info.ext == 0)
		nm_print_absolute_32(MIN, data[item.cur].n_value);
	else if (sy_info.type == N_SECT && sy_info.ext == 1)
		nm_print_sect_32(MAJ, data[item.cur].n_value,
			data[item.cur].n_sect, &sy_info);
	else if (sy_info.type == N_SECT && sy_info.ext == 0)
		nm_print_sect_32(MIN, data[item.cur].n_value,
			data[item.cur].n_sect, &sy_info);
	else if (sy_info.type == N_INDR)
	{
		...
		nm_display_symbol_value_type_32(data, new_item, m_header, info);
	}
	else
		nm_print_unknown_32();
}
